...
class BookDocument(Base):
    """A private PDF source imported once and reused by PDF-backed book courses."""
    ...
    status = Column(String(50), nullable=False, default="queued")  # queued / processing / ready / failed
    error = Column(Text, nullable=True)
    page_count = Column(Integer, nullable=True)
    extracted_page_count = Column(Integer, nullable=True)
    toc_source = Column(String(50), nullable=True)
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    updated_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))

    user = relationship("User")
    chapters = relationship("BookDocumentChapter", back_populates="document", cascade="all, delete-orphan", order_by="BookDocumentChapter.order_index")
    courses = relationship("Course", back_populates="source_document")


class BookDocumentChapter(Base):
    """A chapter range resolved from the imported PDF's own table of contents."""
    __tablename__ = "book_document_chapters"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    document_id = Column(UUID(as_uuid=True), ForeignKey("book_documents.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    document = relationship("BookDocument", back_populates="chapters")
    chunks = relationship("BookDocumentChunk", back_populates="chapter", cascade="all, delete-orphan", order_by="BookDocumentChunk.order_index")
    note = relationship("Note")
    units = relationship("CourseUnit", back_populates="source_chapter")


class BookDocumentChunk(Base):
    """A bounded, page-labelled PDF passage used for grounded book-chat retrieval."""
    __tablename__ = "book_document_chunks"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    chapter_id = Column(UUID(as_uuid=True), ForeignKey("book_document_chapters.id", ondelete="CASCADE"), nullable=False, index=True)
    page_start = Column(Integer, nullable=False)
    page_end = Column(Integer, nullable=False)
    order_index = Column(Integer, nullable=False, default=0)
    content = Column(Text, nullable=False)

    chapter = relationship("BookDocumentChapter", back_populates="chunks")
...
class Course(Base):
    ...
    source_document_id = Column(UUID(as_uuid=True), ForeignKey("book_documents.id", ondelete="SET NULL"), nullable=True)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    updated_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))

    user = relationship("User", back_populates="courses")
    source_document = relationship("BookDocument", back_populates="courses")
...
class CourseUnit(Base):
    ...
    source_chapter_id = Column(UUID(as_uuid=True), ForeignKey("book_document_chapters.id", ondelete="SET NULL"), nullable=True)
    ...
    order_index = Column(Integer, nullable=False, default=0)
    sections = Column(JSON, nullable=True)  # [{"title": str, "focus": str}] — sub-steps within a lesson
    current_section = Column(Integer, nullable=False, default=0)  # index of the section currently being taught
    ...
    source_chapter = relationship("BookDocumentChapter", back_populates="units")
