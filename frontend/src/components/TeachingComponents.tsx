...
// now (German, warm, generated by the Flash model on the backend). Cycles through
// an array of phrases every ~3s while the model is working.
export function ThinkingStatus({ phrases, phraseIndex, accent }: {
    phrases: string[];
    phraseIndex: number;
    accent: Accent;
}) {
    const a = ACCENTS[accent];
    const current = phrases.length > 0
    ...
    return (
        <div className="flex items-center gap-2.5 text-xs text-dark-400">
            ...
            <span key={`${phraseIndex}-${current}`} className="status-line italic">
                {current}
            </span>
        </div>
    );
